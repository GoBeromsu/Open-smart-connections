...
import type { AdapterDefaults, SettingsConfigEntry, ModelOption } from '../../types';

/**
 * Simple adapter name-to-class mapping for request/response adapters.
 * Add or remove mappings here as needed.
 */
const adapters_map: Record<string, {req: typeof SmartChatModelRequestAdapter, res: typeof SmartChatModelResponseAdapter}> = {
  'openai': {
...
export class SmartChatModelCustomAdapter extends SmartChatModelApiAdapter {
   ...
   */
  get req_adapter(): typeof SmartChatModelRequestAdapter {
    const adapter_name: string = this.model.data.api_adapter || 'openai';
   ...
   */
  get res_adapter(): typeof SmartChatModelResponseAdapter {
    const adapter_name: string = this.model.data.api_adapter || 'openai';
    const map_entry = adapters_map[adapter_name];
   ...
   */
  get endpoint(): string {
    const protocol: string = this.model.data.protocol || 'http';
    const hostname: string = this.model.data.hostname || 'localhost';
    const port: string = this.model.data.port ? `:${this.model.data.port}` : '';
    let path: string = this.model.data.path || '';
    if (path && !path.startsWith('/')) path = `/${path}`;
    return `${protocol}://${hostname}${port}${path}`;
  }

  get_adapters_as_options(): ModelOption[] {
   ...
   * @override
   * @returns {Record<string, SettingsConfigEntry>} settings configuration
   */
  get settings_config(): Record<string, SettingsConfigEntry> {
    return {
      ...
      '[CHAT_ADAPTER].api_adapter': {
        ...
        options_callback: (() => { this.get_adapters_as_options(); }) as any, // UNTESTED
        default: 'openai',
      },
      '[CHAT_ADAPTER].id': {
        name: 'Model Name',
