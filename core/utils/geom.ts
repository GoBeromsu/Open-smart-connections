/**
 * @file geom.ts
 * @description Geometric operations on N-dimensional points
 */

/**
 * Computes the centroid of an array of points in N-dimensional space.
 * The centroid is the arithmetic mean of coordinates across all points.
 *
 * @param points Array of points, each is an array of numbers
 * @returns The centroid as an array, or null if no points
 */
export function compute_centroid(points: number[][]): number[] | null {
  ...
  // Use typed array for efficiency
 ...
 *
 * @param points Array of points, each is an array of numbers
 * @returns The medoid point as an array, or null if no points
 */
export function compute_medoid(points: number[][]): number[] | null {
  if (!points || points.length === 0) {
    return null;
  }
  ...
  // Track sum of distances for each point
  const sum_of_distances = new Float64Array(n);

  // Compute each pairwise distance once
  for (let i = 0; i < n - 1; i++) {
    const p_i = points[i];
    for (let j = i + 1; j < n; j++) {
      const p_j = points[j];
