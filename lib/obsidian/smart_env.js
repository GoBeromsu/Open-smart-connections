...
export class SmartEnv extends BaseSmartEnv {
 ...
 refresh_status() {
  ...
  get embedding_models() {
    if (!this._embedding_models) {
      try {
        const embed_settings = this.settings.smart_sources?.embed_model
          ?? this._config?.default_settings?.smart_sources?.embed_model
          ?? { adapter: 'transformers', transformers: { model_key: 'TaylorAI/bge-micro-v2' } };

        const embed_model = this.init_module('smart_embed_model', {
          settings: embed_settings,
          env: this,
          re_render_settings: () => this.render_settings?.(),
          reload_model: () => { this._embedding_models = null; },
        });

        if (embed_model) {
          this._embedding_models = { default: { instance: embed_model } };
        }
      } catch (e) {
        console.warn('Failed to initialize embedding_models:', e);
      }
    }
    return this._embedding_models;
  }
  // Smart Plugins (removed - external service integration)
  /**
